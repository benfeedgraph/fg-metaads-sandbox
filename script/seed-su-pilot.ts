 ...
 * as product_level and pass the SKU-insights gate. A FeedGraph sync therefore reproduces the same
 * per-SKU numbers as the direct FeedGraph seed.
 *
 * Idempotent. Run from sandbox-meta:  npx tsx script/seed-su-pilot.ts
 */
...
function main() {
  ...
  console.log(`Loaded ${canon.length} pilot products (${withMeta.length} with Meta activity).`);
  ...
  const tx = db.transaction(() => {
    ...
    // dedicated catalog-sales campaign/adset/ad (objective drives FeedGraph product-level classification)
    ...
    // products (issues preserved: blank fields stay blank)
    ...
    // product sets: simple contains match on product_type / custom_label_0 / brand
    ...
    // one insight per SKU (distinct product_id → no breakdown inflation), real totals
    let rows = 0;
  ...
  const count = (db.prepare("SELECT COUNT(*) c FROM catalog_products").get() as { c: number }).c;
  const withIns = (db.prepare("SELECT COUNT(DISTINCT product_id) c FROM insights").get() as { c: number }).c;
  const tot = db.prepare("SELECT ROUND(SUM(spend),0) spend, ROUND(SUM(purchase_value),0) rev FROM insights").get();
  console.log(`\nMeta sandbox catalog ${catalogId}: ${count} products, ${rows} insight rows (${withIns} SKUs).`);
  console.log("totals:", JSON.stringify(tot));
  console.log("sample:", JSON.stringify(db.prepare("SELECT retailer_id, name, price FROM catalog_products LIMIT 3").all()));
  process.exit(0);
}
main();
